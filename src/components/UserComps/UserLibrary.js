...
/**
 * fetch books not returned by user
 * @param page
 * @returns {*}
 */
...
class UserLibrary extends Component {
		...
		/**
		 * goes to next page by updating page state
		 */
		handleNextPage = () => {
		...
		/**
		 * goes to previous page by updating page state
		 */
		handlePrevPage = () => {
				this.setState(currentState => currentState.pageNum--, () => this.fetchBooks());
		};
		...
		/**
		 * handles user book return by sending id to API via Axios
		 * @param id
		 * @param bookTitle
		 */
